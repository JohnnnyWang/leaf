...
use super::aead_helper::{random_iv_or_salt, AeadCipherHelper, BlockCipherHelper};
...
impl VmessHeaderReader {
    ...
    pub fn decrypt_resp_header_len_in_place_with_slice(
        &self,
        aad: &[u8],
        buffer: &mut [u8],
    ) -> bool {
        self.resp_header_len_enc
            .decrypt_in_place_with_slice(&self.header_len_iv, aad, buffer)
    }

    pub fn decrypt_resp_header_payload_in_place_with_slice(
        &self,
        aad: &[u8],
        buffer: &mut [u8],
    ) -> bool {
        self.resp_header_payload_enc
            .decrypt_in_place_with_slice(&self.header_payload_iv, aad, buffer)
    }
